...
private(
	...
	errvt 		method(List, Remove, len_t index, len_t len);
	...
	std_List*	method(List, SubList, std_List* dest, u64 index, len_t len);
...
/**
@def createList(ptr, type, ...)
@brief allocates a static buffer object on the stack 
*/
#define createList(_type, _ptr, _size) create(std_List, _ptr, _size, T(_type))
/**
@def newList(type, ...)
@brief allocates a static buffer object on the stack 
*/
