...
#define fmtStr std_Types_Format_Args

#define f (*format)
#define fmt(...) (std_Types_Format_Args){(pntr[]){__VA_ARGS__}, sizeof((pntr[]){__VA_ARGS__}) / sizeof(pntr)}
...
#define printTo(strm, ...) 	std.Types.data.writeTo( 					\
				    V(strm), 							\
				    (const void*[]){__VA_ARGS__ __VA_OPT__(,) fmt_end}, 	\
