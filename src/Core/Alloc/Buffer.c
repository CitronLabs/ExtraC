...
u64          moduleIMethod(Allocator_getBytesAlloced){ self(std_Buffer); return priv.type.size * priv.size; }
bool         moduleIMethod(Allocator_isStatic){ self(std_Buffer); return priv.isStatic; }

errvt moduleIMethod(Allocator_setMax, u64 max){ 
    self(std_Buffer); 
...
COPY(std_Buffer){
    ...
    void* copy_loc = std.Buffer.Allocator.Alloc(where, priv.size, getErrorPos());
